...
export const useRealtimeSession = () => {
  ...
  const togglePause = async (personId: string) => {
    ...
    try {
      ...
      // Actualizar estado de pausa (no redistribuir nada)
      const { error: updateError } = await supabase
        .from('session_people')
      ...
      toast({
        ...
        description: newPausedState 
          ? `${person.name} está em pausa e não receberá novos APPs.`
          : `${person.name} voltou ao trabalho e receberá novos APPs.`,
      });
    } catch (error) {
